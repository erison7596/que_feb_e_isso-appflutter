  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: FlutterSplashScreen.gif(
        gifPath: 'assets/app/ricardo.gif',
        gifWidth: 269,
        gifHeight: 474,
        nextScreen: MyHomePage(title: 'Que feb é isso?'),
        duration: const Duration(milliseconds: 4515),
        onInit: () async {
          debugPrint("onInit");
          // Aqui você pode fazer qualquer inicialização necessária antes da tela principal ser exibida
        },
        onEnd: () async {
          debugPrint("onEnd 1");
          // Aqui você pode realizar ações após a exibição da tela de introdução (splash screen)
        },
      ),
    );
  }
...
// Options to configure the detector while using with base model.
    final modelPath = await _getModel('assets/ml/1.tflite');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text("Que feb é isso?"),
    ...
    for (DetectedObject detectedObject in objects) {
      ...
      for (Label label in list) {
        printOnCanvas(
          ...
          detectedObject.boundingBox.left * scaleX,
          detectedObject.boundingBox.top * scaleY -
              20, // Ajuste vertical para exibir acima do retângulo
          Colors.white,
        );
        break;
      }
